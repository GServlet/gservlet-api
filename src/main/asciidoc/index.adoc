...
For an exhaustive list of the supported methods, please read the https://gservlet.org/javadocs/1.0.1/org/gservlet/abstractservlet[Javadocs].
...
For an exhaustive list of the supported methods, please read the https://gservlet.org/javadocs/1.0.1/org/gservlet/abstractfilter[Javadocs]. 
...
class MyServletContextAttributeListener {
...
@RequestListener
class MyServletRequestListener {
...
class MyServletRequestAttributeListener {
...
class MyHttpSessionAttributeListener {
...
This interface can be implemented by a class to get notified when its instance is added to the session or when it is removed from the session.
...
@WebListener
public class MyHttpSessionBindingListener implements HttpSessionBindingListener, Serializable {
...
class MyHttpSessionActivationListener {
...
To enable Spring autowiring, you can use an implementation of the https://gservlet.org/javadocs/1.0.1/org/gservlet/scriptlistener[ScriptListener] interface like this:
...
NOTE: Your Groovy scripts must be created in the *_src/main/resources/groovy_* folder if your application is packaged as a jar.
...
Copyright @2021. Free use of this software is granted under the terms of the https://www.apache.org/licenses/LICENSE-2.0[Apache 2.0] License.
