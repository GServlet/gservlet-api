...
The most commonly used approach for creating XML with Groovy is to use a builder. An implicit _xml_ variable bound to an instance of a MarkupBuilder has been made available to your servlets and filters for that purpose.
...
property name : 'xml', type : groovy.xml.MarkupBuilder, provider : 'org.gservlet.AbstractServlet'
}
...
contributor(classContext) {
  if (hasAnnotation('org.gservlet.annotation.Servlet')) {
    ...
    property name: 'xml', type: 'groovy.xml.MarkupBuilder'
