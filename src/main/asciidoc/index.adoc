...
. Create the scripts folder inside your web app directory
...
Make sure your project has the Groovy nature and the Groovy DSL support is activated. Also to leverage the full power of the https://marketplace.eclipse.org/content/groovy-development-tools[Groovy Development Tools (GDT)], it is highly recommended to add your _scripts_ folder as source folder. Thus, you will be able to use its wizards to create your artifacts and like for Java, the use of the default package is discouraged. The example below illustrates a good practice to follow throughout your development in order to structure your project.
...
The _scripts_ folder must be created inside the _src/main/resources_ directory of your Spring Boot applications so it can be packaged inside the final jar and you just need this configuration below to leverage the GServlet API. 
