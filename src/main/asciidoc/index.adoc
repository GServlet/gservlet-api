...
If the target framework supports the _@WebListener_ annotation of the Servlet API, no configuration is required since we ship with a context listener which starts and stops the application. This statement is valid for frameworks like JSF, Apache Struts.
